...
scaled.df$Category.Label <- fb$Category
scaled.df$Month.Label <- month.abb[fb$Post.Month]
scaled.df$Weekday.Label <- day.abb[fb$Post.Weekday]
...
server <- function(input, output) {
  ...
  output$parallel <- renderPlot({
    ggparcoord(parallels(), columns = c(3,4,5,6), scale = "uniminmax", mapping = aes(colour = "blue")) +
      ...
      geom_label(data = scaled.df, aes(x = 1, y = Category, label = Category.Label), inherit.aes = FALSE, size = 6, label.r=unit(0, "lines"), label.padding=unit(0.1, "lines"), color="white") + 
      annotate("text", x = rep(1,3), y = seq(0,1,1/2), label = c(1:3), size = 6) +
      geom_label(data = scaled.df, aes(x = 2, y = Post.Month, label = Month.Label), inherit.aes = FALSE, size = 6.5, label.r=unit(0, "lines"), label.padding=unit(0.1, "lines"), color="white") + 
      annotate("text", x = rep(2,12), y = seq(0,1,1/11), label = month.abb[1:12], size = 6.5) +
      geom_label(data = scaled.df, aes(x = 3, y = Post.Weekday, label = Weekday.Label), inherit.aes = FALSE, size = 6.5, label.r=unit(0, "lines"), label.padding=unit(0.1, "lines"), color="white") + 
